  ...
  <NuxtLayout name="sidebar">
    <v-container fluid class="pa-8">
      <app-listing
        ...
        "
        @update:page="
          sharedStore.getListingItems('commentList', payload, headers)
        "
      ...
      >
        ...
        <template #relatedCourse="{ item }">
          <div class="d-flex cursor-pointer">
            <span class="text-truncate text-body-1">{{
              item.item.relatedCourse?.title
            }}</span>
          </div>
        </template>
...
let headers = ref([
  ...
  { key: "date", title: t("date"), sortable: true, size: "200px" },
  { key: "relatedCourse", title: t("relatedCourse"), size: "100px" },
...
const publish = () => {
  ...
  $repos.academyPanel.updateCommentStatus(payload).then((res) => {
...
onMounted(async () => {
  ...
  await sharedStore.getListingCommon("commentListCommon", "academyPanel");
  init();
  await sharedStore.getListingItems(
    "commentList",
    payload.value,
