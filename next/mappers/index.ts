...
interface Mapping {
  [key: string]: {
    newName?: string;
    defaultValue?: any;
    processValue?: (value: any, object: any, collectionMapping: any) => any;
    setValue?: (value: any, mapping: any, raw: boolean) => any;
  };
}
...
function mapper(
  ...
  for (const key in object) {
    if (mapping.hasOwnProperty(key)) {
      ...
      if (object[key] && mapping[key].hasOwnProperty("processValue")) {
        result[newKey] = mapping[key].processValue(
          object[key],
          object,
          collectionMapping
        );
  ...
  for (const key in mapping) {
    ...
    if (mapping[key].hasOwnProperty("setValue")) {
      result[mappingKey] = mapping[key].setValue(
        object,
        collectionMapping,
        raw
      );
