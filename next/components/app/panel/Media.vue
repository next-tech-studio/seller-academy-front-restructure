<template>
  <div>
    ...
    <v-card v-else rounded="lg" :height="height">
      <v-img v-if="type == 'image'" :src="modelValue.filePreview"></v-img>
      <app-video-player
        v-if="type == 'video'"
        :video-src="modelValue.filePreview"
        :video-poster="modelValue.coverPreview"
      ...
      <app-audio-player
        v-if="type == 'audio'"
        :audio-src="modelValue.filePreview"
        :audio-poster="modelValue.coverPreview"
      />
    </v-card>
...
let coverValue = computed({
  ...
  set(newValue) {
    ...
    createPreview(newValue[0], "coverPreview");
  },
});
...
let fileValue = computed({
  ...
  set(newValue) {
    ...
    createPreview(newValue[0], "filePreview");
  },
});

let uploaded = computed(
  () => !!(props.modelValue.filePreview || props.modelValue.coverPreview)
);
