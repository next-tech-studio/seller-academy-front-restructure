<template>
  <app-switch-wrapper>
    ...
    <div v-else class="d-flex">
      <div v-for="(item, index) in items" :key="index" :value="item">
        ...
        <div v-if="item.type === 'datePicker'">
          ...
          >
            ...
            class="text-body-1"
              :class="
                date.length>1 ? 'text-text-primary' : 'text-text-high-emphasis'
              "
              >{{ $t(item.title) }}</span
            >
          </v-btn>
      ...
      <v-btn v-if="items.length && removeAll" color="text-low-emphasis" variant="text" append-icon="custom:x" @click="removeFilters()">حذف همه‌ی فیلتر‌ها</v-btn>
    </div>
  </app-switch-wrapper>
</template>
