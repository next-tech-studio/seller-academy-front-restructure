  ...
  <NuxtLayout name="sidebar">
    <v-container fluid class="pa-8">
      <app-listing
        ...
        :show-dialog="true"
        :add-new-item="true"
        dialog-start-button-title="add_new_tag"
        :dialog-title="sharedStore.edit? 'edit_tag':'add_new_tag'"
        ...
        "
        @update:page="
          sharedStore.getListingItems('tagList', payload, headers.value)
        "
        ...
        @filter="
          sharedStore.getListingItems('tagList', payload, headers, 'panel')
        "
      >
...
const submitItem = () => {
  ...
  if (sharedStore.edit) {
    ...
    $repos.panel.updateTag(payload).then((res) => {
...
onMounted(async () => {
  await sharedStore.getListingCommon("tagListCommon", "panel");
  init();
  await sharedStore.getListingItems(
    "tagList",
    payload.value,
